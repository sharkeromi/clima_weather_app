import 'package:clima_weather_app/services/weather.dart';
import 'package:flutter/material.dart';
import 'package:geolocator/geolocator.dart';
import 'package:get/get.dart';

class WeatherController extends GetxController {

  WeatherModel weatherModel = WeatherModel();

  RxDouble temperature = RxDouble(0.0);
  RxString weatherIcon = RxString('');
  RxString cityName = RxString('');
  RxString regionName = RxString('');
  RxString weatherMessage = RxString(''); 
  RxString country = RxString('');
  RxString message = RxString('');

  TextEditingController searchCity = TextEditingController();

  getWeatherData(weatherData) {
    if (weatherData == null) {
      temperature.value = 0.0;
      weatherIcon.value = 'Error';
      weatherMessage.value = 'Unable to get weather data';
      cityName.value = '';
      regionName.value = '';
      return;
    }
    temperature.value = weatherData['current']['temp_c'];
    weatherMessage.value = weatherData['current']['condition']['text'];
    weatherIcon.value = weatherData['current']['condition']['icon'];
    regionName.value = weatherData['location']['region'];
    cityName.value = weatherData['location']['name'];
    country.value = weatherData['location']['country'];
    message.value=weatherModel.getMessage(temperature.value.toInt());
    // weatherMessage.value = weatherModel.getMessage(temperature.value.toInt());
  }

  /// Determine the current position of the device.
  ///
  /// When the location services are not enabled or permissions
  /// are denied the `Future` will return an error.
  Future<Position> determinePosition() async {
    bool serviceEnabled;
    LocationPermission permission;

    // Test if location services are enabled.
    serviceEnabled = await Geolocator.isLocationServiceEnabled();
    if (!serviceEnabled) {
      // Location services are not enabled don't continue
      // accessing the position and request users of the
      // App to enable the location services.
      return Future.error('Location services are disabled.');
    }

    permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied) {
      permission = await Geolocator.requestPermission();
      if (permission == LocationPermission.denied) {
        // Permissions are denied, next time you could try
        // requesting permissions again (this is also where
        // Android's shouldShowRequestPermissionRationale
        // returned true. According to Android guidelines
        // your App should show an explanatory UI now.
        return Future.error('Location permissions are denied');
      }
    }

    if (permission == LocationPermission.deniedForever) {
      // Permissions are denied forever, handle appropriately.
      return Future.error(
          'Location permissions are permanently denied, we cannot request permissions.');
    }

    // When we reach here, permissions are granted and we can
    // continue accessing the position of the device.
    return await Geolocator.getCurrentPosition();
  }
}
